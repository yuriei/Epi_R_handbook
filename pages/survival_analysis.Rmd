...
#Add a figure from the following chuncks for the last version of the page
...
#if(!require(survminer)) remotes::install_github("kassambara/survminer", build_vignettes = FALSE)
...
We start by loading the linelist as you have done previously and then we will use the same procedure to load the Oral cancer data
...
with(linelist_surv, table(outcome, event, useNA = "ifany"))

with(linelist_surv, table(age_cat_small, age_cat, useNA = "ifany")) 
...
survobj <- with(linelist_surv, survival::Surv(futime, event))
...
We then start our analysis using the `survfit()` function to produce a *survfit object*, which fits the default calculations for **_Kaplan Meier_** (KM) estimates of the overall (marginal) survival curve, which are in fact a step function with jumps at observed event times.
...
We can also use the `print()` function. The `print.rmean=TRUE` argument is used to obtain the mean and its standard error.
...
lines(linelistsurv_fit, #draw an additional curve to the previous plot
      fun = "event", #draw the cumulative events instead of the survival 
      mark.time=FALSE, #do not mark times of events to facilitate reading of the curve
...
plot(linelistsurv_fit_sex, col=col_sex, xlab = "Days of follow-up", ylab="Survival Probability")
legend("topright", legend=c("Female","Male"), col =col_sex, lty = 1, cex=.7, bty = "n" )

#compute the test of the difference between the survival curves
survdiff(Surv(futime, event) ~ gender, data = linelist_surv)
...
survminer::ggsurvplot(
  ...
  linelistsurv_fit_sex, data= linelist_surv, size=1, 
...
legend.title = "Gender", legend.labs = c("Female","Male"), font.legend = 10, #legend characteristics
...
linelistsurv_fit_source <-  survfit(Surv(futime, event) ~ source, data = linelist_surv)
...
#fitting the model
linelistsurv_cox_sexage <-  survival::coxph(Surv(futime, event) ~ gender + age_cat_small, data = linelist_surv)
...
[Cahpter on advanced survival models Princeton](https://data.princeton.edu/wws509/notes/c7.pdf)
