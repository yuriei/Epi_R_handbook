...
This page demonstrates the use of **janitor**, **dplyr**, **gtsummary**, and **base** R to summarise data and *produce* tables and descriptive statistics. 

Each of these tools have advantages and disadvantages in the areas of code simplicity, accessibility of outputs, quality of printed outputs. Use this page to decide which approach works for your scenario.  

*See the [Tables for presentation] page for tips on how to nicely format and print your tables.*  
...
You have several choices when producing tabulation and cross-tabulation summary tables. Some of the factors to consider include code simplicity and ease, the desired output (printed to R console, or as "pretty" .png/.jpeg/.html image), and what you can do with the data afterward. Consider the points below as you choose the tool for your situation.
...
You can use **janitor**'s `adorn_*()` functions on other tables, such as those created by `summarise()`, `count()`, or `table()`. Simply pipe the table to the desired **janitor** function. For example:  
...
If you convert the table to a "pretty" image with a package like **flextable**, you can save with its functions `save_as_html()`, `save_as_word()`, `save_as_ppt()`, and `save_as_image()`, as discussed more extensively in the [Tables for presentation] page. Below, the table is saved as a Word document, in which it can be further hand-edited.  
...
* You may have created column for the purposes of calculation (e.g. denominators) that you eventually drop from your final data frame (e.g. with `select()`).  
...
max(column[criteria], na.rm=T)
min(column[criteria], na.rm=T)

sum(column == value, na.rm=T)
sum(column %in% c(), na.rm=T)


### Glueing together {.unnumbered}  

The function `str_glue()` from **stringr** is useful to combine columns into one new column - typically used *after* the `summarise()` command.  
...
Now you could pass to **flextable** and print the table to Word, .png, .jpeg, .html, Powerpoint, RMarkdown, etc.! (see the **janitor** section above and the [Tables for presentation] page).  
...
You can use summarise across multiple columns using `across()`. This makes life easier when you want to calculate the same statistics for many columns. To specify which columns to operate across, either:  

* provide `.cols = ` as either a vector of column names `c()` or "tidyselect" helper functions (explained below)  
* provide `.fns = ` the function to perform (no parentheses) - you can provide multiple within a `list()`
...
Here are those "tidyselect" helper functions that you can place *within* `across()`:  
...
The example below begins with the "long" table `age_by_outcome` from above. We create it again and print, for clarity:  
...
In the wider version, the new column names are specified to `names_from = ` while the values are specified to come from column `n`. The columns not mentioned (`outcome` in this case) will remain unchanged on the far left.  
