...
Most of the abbreviations are listed below. You can see the complete list by running `?strptime`.  
...
Using `format()` will convert the values to class Character, so this is generally used towards the end of an analysis or for display purposes only! You can see the complete list by running `?strptime`.  
...
See the page on [Grouping data] for example of grouping by date. Below we briefly describe grouping data by weeks.  
