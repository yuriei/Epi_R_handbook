...
You can print the file path to your library by entering `.libPaths()` (empty parentheses). This becomes especially important if working on network drives.   
...
The syntax quite simple. Just list the names of the packages within the `p_load()` parentheses, separated by commas. If someone on another computer runs the script, this command will install the **rio**, **tidyverse**, and **here** packages if they are not yet installed, and will load them for use. Note that package names are case-sensitive. 
...
Note that we have used the syntax `pacman::p_load()` which explicitly writes the package name prior to the function name, connected by two colons `::`. This syntax is useful because it also loads the **pacman** package (assuming it is already installed).  

There are alternative **base** R functions that you will see often. The **base** R function for installing a package is `install.packages()`. The name of the package to install must be provided *in quotes*. If you want to install multiple packages, they must be listed within a character vector `c()`.    
...
Installation can also be accomplished point-and-click by going to the RStudio "Packages" pane and clicking "Install" and typing the package name.  

The **base** R function to **load** a package for use (after it has been installed) is `library()`. It loads only one package at a time (another reason to use `p_load()`). You can provide the package name with or without quotes.    
