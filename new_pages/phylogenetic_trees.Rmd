...
Note: to illustrate which nodes we are manipulating we first apply the **geom_hilight()** function to highlight the samples in the nodes we are interested in and store that ggtree plot object in a new object `p1`.

```{r, phylogenetic_trees_rotating_2, eval=F, out.width=c('50%'), fig.show='hold', fig.align='center'}

p1 <- p + geom_hilight(39, "steelblue", extend = 0.0017) +  # highlights the node 39 in blue, "extend =" allows us to define the length of the color block
geom_hilight(37, "yellow", extend = 0.0017) +               # highlights the node 37 in yellow
