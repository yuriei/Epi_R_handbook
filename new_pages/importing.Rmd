...
An "Automated Programming Interface" (API) is commonly used to directly request data from a major website (e.g. Google, Twitter, US CDC, etc). You can import data directly via an API with the R packages **httr** and **jsonlite**.  

APIs are a set of rules that allow one software application to interact with another. The client (you) sends a "request" and received a "response" containing content.  

Some sites that use APIs require authentication (e.g. a platform with user ID and credentials) and others that anyone can access without credentials. Needless to say, it is necessary to have an internet connection to import data via API.  

We will briefly give examples, and link you to further resources.  


### HTTP request {.unnumbered}  

The API exchange is most commonly done through an HTTP request. The exact input and output may vary depending on the type of API but the process is the same - a "Request" (often HTTP Request) from the user, followed by a "Response".  

There are a few components of an *HTTP request*, including:  

* The URL of the API endpoint  
* The Verb/Method (most likely, when pulling data into R this will be "GET")  
* Headers  
* Body  

After your request, you will receive a "response" in an similar format, including URL, HTTP status (Status 200 is what you want!), file type, size, and the desired content. You will then need to parse this response and turn it into a workable data frame within your R environment.


### Packages {.unnumbered}  

The **httr** package works well for handling HTTP requests in R, even for users who are less familiar with software development terminology. It requires little prior knowledge of Web APIs.  

In addition, if the response is .json, you can use **jsonlite** to parse the response.  
...
Below is an example of an HTTP request from [The Trafford Data Lab](https://www.trafforddatalab.io/open_data_companion/#A_quick_introduction_to_APIs) which has a number of other easy-to-follow API exercises.

Example: The Food Standards Agency provide food hygiene rating data for the United Kingdom. We want to get a list of fast food outlets in Trafford.
...
Below, see the `response` data frame of fast food outlets. You can now clean it and use it! 

```{r, echo=F}
DT::datatable(response, rownames = FALSE, options = list(pageLength = 5, scrollX=T), class = 'white-space: nowrap' )
```
...
Some APIs require authentication - for you to prove who you are, so you can access restricted data. To import these data, you may first use a POST method to provide a username, password, or code. This will return an access token, that can be used for subsequent GET method requests to retrieve the desired data.  

Below is an example of querying data from *Go.Data*, which is an outbreak investigation tool. *Go.Data* uses an API for all interactions between the web front-end and smartphone applications used for data collection. *Go.Data* is used throughout the world. Because outbreak data are very sensitive and you should only be able to access data for *your* outbreak, authentication is required. Below is some sample R code using **httr** and **jsonlite** for connecting to the *Go.Data* API to import data on contact follow-up.  
...
Below we retrieve the raw Go.Data contact list for further analysis (NOTE: this is from DEMO Sandbox instance).

```{r out.width = "100%", fig.align = "center", echo=F}
knitr::include_graphics(here::here("images", "godata_api_contactsdf.png"))
```
