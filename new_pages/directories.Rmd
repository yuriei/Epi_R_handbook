...
```{r}
# file paths
dir_ls(here("data", "population"))
```

To get all the metadata information about each file in a directory, (e.g. path, modification date, etc.) you can use `dir_info()` from **fs**. This can be particularly useful if you want to extract the last modification time of the file, for example if you want to import the most recent version of a file. For an example   

```{r, eval=F}
# file info
dir_info(here("data", "population"))
```

Here is the data frame returned. Scroll to the right to see all the columns.  

```{r, echo=F}
DT::datatable(dir_info(here("data", "population")), rownames = FALSE, options = list(pageLength = 5, scrollX=T), class = 'white-space: nowrap' )
```
...
```{r, echo=F}
DT::datatable(file_info(here("data", "linelist_cleaned.rds")), rownames = FALSE, options = list(pageLength = 5, scrollX=T), class = 'white-space: nowrap' )
```
...
```{r}
is_file(here("data", "linelist_cleaned.rds"))
```
...
Here is an example of creating a folder if it does not already exist:  
...
See the page on [R Markdown reports] for more details. Also see the documentation for `render()` [here](https://rmarkdown.rstudio.com/docs/reference/render.html) or by entering `?render`.  
