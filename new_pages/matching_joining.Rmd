...
See other worked examples like this in the [Descriptive tables] and [HTML tables] pages.  
...
Now we can view the result. See how in the final row, an empty `NA` value fills in for the column `hospital` that was not in `totals`. As explained in the [HTML tables] page, you could "fill-in" this value with "Total" using `replace_na()`.  
