...
Each page is stored within its own Rmarkdown file in the "pages" folder of the R project. Each page can be knitted on it's own, as in this case it will source the `page_setup.Rmd` file to load datasets and settings. The file `handbook_combined.Rmd` contains code that sources/run each of the scripts in the "pages" folder, combining them all into one Rmarkdown html document.  

* If you want to work on your page, open it and edit it, knit it individually and examine your work, and make sure it is included within `handbook_combined.Rmd`.  
...
2) Open the file `_template.Rmd`, save it under a different name  
3) Read the template and edit/add content  

Test your page routinely by knitting it. Test knitting the entire handbook on your local cloned repo. If your page is breaking the handbook (knitting of `handbook_combined.Rmd`) and you can't resolve it, alert the core team and put `eval=F` next to your page so its chunk doesn't run.
...
**`page_cleanup`** - Lastly, there is a chunk that sources the `page_cleanup.Rmd` script, which sets the datasets back to their default values.  
