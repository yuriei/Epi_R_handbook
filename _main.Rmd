...
**`cases`** is our "baseline" dataframe containing patients who are awaiting test results.  


```{r message=FALSE, echo=F}
# display the hospital data as a table
DT::datatable(cases, rownames = FALSE, options = list(pageLength = nrow(cases)))
```

**`test_results`** has more records, and contains column `result`, which we want to add to `cases` based on probabilistic matching of records.  

```{r message=FALSE, echo=F}
# display the hospital data as a table
DT::datatable(results, rownames = FALSE, options = list(pageLength = nrow(results)))
```

### Probabilistic matching  

The `fastLink()` function from the **fastLink** package can be used to apply a matching algorithm. Here are some of the useful functions:  

* Define the two dataframes as `dfA` and `dfB`  
* Give all column names to be used for matching to `varnames = ` - these columns must exist in both `dfA` and `dfB`.  
* Give columns names to be evaluated on string "distance" to `stringdist.match = `. They must also be in `varnames`.  
* Give the names of columns to be evaluated on numeric matching to `numeric.match = `. They must also be in `varnames`.  
* Missing values are ignored  
* By default, each row in either dataframe is matched to at most one row in the other dataframe. If you want to see all the evaluated matches, set `dedupe.matches = FALSE`. The deduplication is done using Winkler's linear assignment solution (see function help for details).  
...
Below, the data are matched on string distance across the name and district columns, and on numeric distance for year, month, and day of birth. A threshold of 95% probability is set.  
...
The object returned from `fastLink()` is of class `list`, so it actually contains several dataframes within it. One of these dataframes is `matches`, which contains the links.  

When `matches` is printed, you see two column vectors: the pairs of row numbers (also called "rownames") in `dfA` and in `dfB` representing the best matches. If a row number from `dfA` is missing, then no match was found in dfB at the specified match threshold.    
...
* One row from `dfA` ("Blessing Adebayo") had no good match, row number 9 is not present in `matches`.  
...
1) Use `left_join()` to join `matches` to `cases` (matching rownames in `cases` to "inds.a" in `match`)  
...
* Both the columns in `match` are converted to class character, so they can be joined to the character rownames  
...
```{r message=FALSE, echo=F}
DT::datatable(complete, rownames = FALSE, options = list(pageLength = nrow(complete)))
```

Alternatively, before joining you can use `select()` on `results` so that it contains only rownames and any columns that you want to add to `cases` (e.g. the column `result`).  
...
results_not_matched <- results[!rownames(results) %in% matches$inds.b,]  # Rows in results that did NOT match to a row in cases
```


### Probabilistic deduplication  

Probabilistic matching can be used to deduplicate a dataset as well. See the page on deduplication for many methods of deduplication.  

Here we begin with the `cases` dataset, but with 2 rows that are likely duplicates (not 100% duplicates): "Tony" with "Anthony", and "Marialisa Rodrigues" with "Maria Rodriguez".  
...
```
Below we run the same `fastLink()` command as before, but the dataframe is compared to itself. When the two datasets are identical, the function assumes you want to deduplicate.  
...
`fl.out` must be of class `fastLink.dedupe`, or in other words, the result of either `fastLink()` or matchesLink().  

Now, you can review the potential duplicates with `getMatches()`. Provide the dataset for both `dfA` and `dfB`, and provide the output of the `fastLink()` function to `fl.out`.  
...
See the left-most column, which indicates the duplicate IDs - the final two rows are identified as being likely duplicates of rows 2 and 3.  

```{r message=FALSE, echo=F}
DT::datatable(cases_dedupe, rownames = FALSE, options = list(pageLength = nrow(cases_dedupe)))
```

To return the rows which are likely duplicates:  
...
And to inspect the whole rows of the likely duplicates, put the row number in this command:  
