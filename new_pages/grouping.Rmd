...
To summarise the counts in column `n` by outcome you can group by outcome and use `sum(n, na.rm=T)`. To summarise the counts by outcome *and within that by gender subset*, you can use the subset bracket [ ] syntax on the count column as below:  
