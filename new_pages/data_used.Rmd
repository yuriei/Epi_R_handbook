...
### Option 1: Install via R package {-}  

Run the following code to install our R package **handbookfiles** from the Github repository *epirhandbook*. This package in not on CRAN, so use the special function `p_install_gh()` to install it from Github.  

```{r, eval=F}
pacman::p_install_gh("epirhandbook/handbookfiles")
```

Now use the function `download_book()` to download the book to your computer. By default a pop-up windown will appear, allowing you to select a save location. Alternatively, you can provide a file path to `save_loc = `.  
...
### Option 1: Use the **handbook_files** package  

You can use our R package **handbook_files** to save the data files to your computer. The package will do the work of grabbing the files from our Github repository.  
