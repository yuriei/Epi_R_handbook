...
If you convert the table to HTML with a package like **flextable**, you can save with its functions `save_as_html()`, `save_as_word()`, `save_as_ppt()`, and `save_as_image()`, as discussed more extensively in the [Tables] page. Below, the table is saved as a Word document, in which it can be further hand-edited.  
...
**dplyr** is part of the **tidyverse** packages and is an extremely common data management tool. Creating tables with **dplyr** functions `summarise()` and `count()` is useful if you are calculating summary statistics, summarizing *by group*, or passing the results to `ggplot()`. 

`summarise()` creates a *new, summary data frame*. If the data are *ungrouped*, it will return a one-row dataframe with the specified summary statistics. If the data are *grouped*, the new data frame will have one row per *group* (see [Grouping data] page).  
...
linelist %>%                 # begin with linelist
  summarise(n_rows = n())    # return new summary dataframe with column n_rows, which returns the number of rows
```
...
Proportions can be added by piping the table to `mutate()` to create a new column. The new column can divide the counts column (`n` by default) over the `sum()` of that column. This will return a proportion. To convert to percents, you can wrap the result in the function `percent()` from the package **scales** (a **base** R package).  
...
You can also use `str_glue()` to combine columns into one new column - typically used *after* the `summarise()` command. Below, the `summary_table` data frame created above is mutated such that columns `delay_mean` and `delay_sd` are combined and their old columns removed.  
...
Now you could pass to **flextable** and print the table to Word, HTML, Powerpoint, RMarkdown, etc.! (see the **janitor** section above and the [Tables] page).  
...
To sum the counts (e.g. in column `n`) by group you can use `summarise()` as above but set the new column equal to `sum(n, na.rm=T)`.  
To add a subset criteria to the sum operation, you can use the subset bracket [ ] syntax on the counts column as below to create male and female columns.  
...
Below, `mean()` is applied to several numeric columns. The columns named, and the single function `mean` is specified (no parentheses). Any additional arguments for the function (e.g. `na.rm=TRUE`) are provided afterwards.  
