...
### Option 1: Download via our R package {-}  

First, run the following code to install our R package **handbookfiles** from the Github repository *epirhandbook*. This package in not on CRAN, so use the special function `p_install_gh()` to install it from Github.  

```{r, eval=F}
pacman::p_install_gh("epirhandbook/handbookfiles")
```

Now, run the function `download_book()` to download the handbook to your computer. By default a pop-up window will appear, allowing you to select a save location. Alternatively, you can provide a file path to `save_loc = `.  
...
If the approach above does not work for you, follow these instructions to download the file firstly from our Github repository.  
...
### Option 1: Use our **handbook_files** package  

UNDER CONSTRUCTION

You can use our R package **handbook_files** to save the data files to your computer. The package will do the work of downloading the files from our Github repository.  


### Option 2: Manually download from Github  

Alternatively, all the data in this handbook can be manually downloaded from the **["data" folder](https://github.com/nsbatra/Epi_R_handbook/tree/master/data)** of our Github repository. When you arrive at this folder, click the file you want to download.    
...
1) Go to the **["data" folder](https://github.com/nsbatra/Epi_R_handbook/tree/master/data/case_linelists)** of our Github repository  
...
2) Open the appropriate subfolder and click on the .csv file of interest  
...
See the page on [Phylogenetic trees]. In Github "data" folder, navigate to the "phylo" subfolder where you can click on the .txt files. Click "Raw", and right-click to download as for the HTML handbook file - save the .txt file on your computer. Then import it with `read.tree()` from the **ape** package.
