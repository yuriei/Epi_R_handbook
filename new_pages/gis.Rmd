...
Using R as a GIS can seem more intimidating at first because instead of "point-and-click", it has a "command-line interface" (you must code to acquire the desired outcome). However, this is a major advantage if you need to repetitively produce maps or have an analysis that is reproducible.  
...
There are a couple of key items you will need to have and to think about to make a map. These include:
  ...
  * If your dataset is not in a spatial format you will also need a **reference dataset**. Reference data consists of the spatial representation of the data and the related attribute table, which would include material containing the location and address information of specific features.
...
You can also combine several different types of visualizations to show complex geographic patterns; for example, the map below shows **health facility catchment areas**:
...
To import a shapefile we use the `read_sf()` function from **sf**. It is provided the filepath via `here()`. - in this case the file is within our R project in the "data", "gis", and "shp" subfolders, with filename "sle_adm3.shp" (see pages on [Import and export] and [R projects] for more information).  
...
select(sle_adm3_dat, admin3name, cases) # print selected variables to console
```
...
ggplot(data=sle_adm3_dat) +
  geom_col(aes(x=fct_reorder(admin3name, cases, .desc=T),   # reorder x axis by descending 'cases'
...
```
The output from the `moran.test()` function shows us a Moran I statistic of `r round(moran_i$estimate[1],2)`. This indicates the presence of spatial autocorrelation in our data - specifically, that regions with similar numbers of Ebola cases are likely to be close together. The p-value provided by `moran.test()` is generated by comparison to the expectation under null hypothesis of no spatial autocorrelation, and can be used if you need to report the results of a formal hypothesis test.
...
To give an example, let's test whether the spatial pattern of Ebola cases from the simulated epidemic is correlated with the spatial pattern of population. To start, we need to add a `population` variable to our existing `sle_adm3` data. We can use the `sle_adm3_pop` dataframe that we created earlier.

```{r}
sle_adm3_dat <- sle_adm3_dat %>% 
  left_join(select(sle_adm3_pop, adm3_pcode, total),  # add population variable 'total'
            by=c("admin3pcod" = "adm3_pcode")) %>%    
  rename(population = total)                          # rename to 'population'
```
...
par(mfrow=c(2,1))
tm_shape(sle_adm3_dat) + tm_polygons("cases") + tm_layout(main.title="Cases")
tm_shape(sle_adm3_dat) + tm_polygons("population") + tm_layout(main.title="Population")
```
...
The output above shows that the Lee's L statistic for our two variables was `r round(lee_test$estimate[1],2)`, which indicates weak negative correlation. This confirms our visual assessment that the pattern of cases and population are not related to one another, and provides evidence that the spatial pattern of cases is not strictly a result of population density in high-risk areas.
