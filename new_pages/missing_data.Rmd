...
If you want to assign a value as `NA`, for example if using `case_when()` (see [Cleaning data] page), there may be circumstances where `NA` is rejected because the column around it is of another class such as Character. The Right-Hand Side (RHS) values in a `case_when()` command must all be of the same class. Thus, if you have character outcomes on the RHS like "Confirmed", "Suspect", "Probable" and `NA` - you will get an error. Instead of `NA`, put "Missing", or else you must put `NA_character_`. Likewise for integers, use `NA_integer_` instead of `NA`. `NA` should work for dates and logical. See the [R documentation on NA](https://stat.ethz.ch/R-manual/R-devel/library/base/html/NA.html) for more information.  
