...
2) The **purrr** package is the **tidyverse** approach to iterative operations - it works by "mapping" a function across many inputs (columns, datasets, etc.)  
...
* The *vector* can be of character values, column names, or a sequence of numbers - these are the values that will change with each iteration. You can use them within the *for loop* operations using the "item" term.  
...
We have chosen the term `hosp` to represent values from the vector `hospital_names`. For the first iteration of the loop, the value of `hosp` will be `r hospital_names[[1]]`. For the second loop it will be `r hospital_names[[2]]`. And so on...  
...
This is a variation on the character sequence above, in which the names of an existing R object are extracted and become the vector. For example, the column names of a data frame. Conveniently, in the operations code of the *for loop*, the column names can be used to *index* their original R object.  
...
In this approach, the sequence is a series of consecutive numbers. Thus, the value of the "item" is not a character value (e.g. "Central Hospital" or "date_onset") but is a number. This is useful for looping through data frames, as you can use the number inside the *for loop* to index the data frame by *row number*.  

For example, let's say that you want to loop over every row in your data frame and extract certain information. Your "items" would be numeric row numbers. Often, "items" in this case are written as `i`.  

The *for loop* process could be explained in words as "for every item in a sequence of numbers from 1 to the total number of rows in my data frame, do X". The first iteration of the loop, `i` would be 1. For the second iteration, `i` would be 2, etc.  

Here is what the sequence looks like in code: `for (i in 1:nrow(linelist)) {}` where `i` represents the item and `1:nrow(linelist)` produces a sequence of consecutive numbers from 1 through the number of rows in `linelist`.  

One advantage of using numbers in the sequence is that is easy to also use the `i` number to index the *container* that stores the loop outputs. There is an example of this in the Operations section below.  
...
If you want the sequence to be numbers but you begin with a vector (not a data frame), use the shortcut `seq_along()` to return a sequence of numbers for each element in the vector. For example, `for (i in seq_along(hospital_names) {}`.  
...
This is code within the *for loop*. You want this code to run for each item in the *sequence*. Therefore, be careful that every part of your code that changes by the item is correctly coded such that it actually changes! E.g. remember to use `[[ ]]` for indexing.  

In the example below, we use iterate through each row in the `linelist`. The `gender` and `age` values of each row are pasted together and stored the container character vector `cases_demographics`. Note how we also use indexing `[[i]]` to save the loop output to the correct position in the "container" vector.  
...
You may want store some plots created by a *for loop* in a list. You actually initialize the container using the same `vector()` command as above, but with `mode = "list"`. Specify the length however you wish.  
...
To test your loop, you can make a temporarily assignment of the item, such as `i <- 10` or `hosp <- "Central Hospital"`. Do this *outside the loop* and then run your operations code only (the code within the curly brackets) to see if the expected results are produced.  
...
We can make an epicurve of *all* the cases using the **incidence2** package as below:  
...
A loop with many iterations can run for many minutes or even hours. Thus, it can be helpful to print the progress to the R console. This `if` statement code can be placed *within* the loop operations to print every 100th number.  
...
* If the function needs no arguments specified, it can be written with no parentheses and no tilde (e.g. `.f = mean`). To provide arguments that will be the same value for each iteration, provide them within `map()` but outside the `.f = ` argument, like the `na.rm = T` in `map(.x = my_list, .f = mean, na.rm=T)`.  
...
The output of using `map()` is a *list* - a type of object that contain other objects within it, each called an "element". So, a list produced by `map()` could contain many data frames, or many vectors, many single values, or even many lists! There are alternative versions of `map()` explained below that produce other types of outputs (e.g. `map_dfr()` to produce a data frame, `map_chr()` to produce character vectors, and `map_dbl()` to produce numeric vectors).  
...
combined <- sheet_names %>% 
     ...
     map_at(.f = ~import(here::here("data", "hospital_linelists.xlsx"), which = .x),
            .at = c(-1))
```
...
You can `View(linelist_split)` and see that this list contains 6 data frames ("tibbles"), each representing the cases from one hospital. 
...
Again we use `map()`: we take the vector of list element names (shown above) and use `map()` to iterate through them, applying `export()` (from **rio** package, see [Import and export] page) on the data frame in the list `linelist_split` that has that name. We also use the name to create a unique file name. Here is how it works:  
...
Here are some examples using the `combined` list created in the section above (containing 6 data frames):  
...
Here we very briefly describe the apply family of functions. We will not describe them extensively, but will explain the syntax sufficiently such that you can know how to interpret the commands if you encounter them.  
...
https://hookedondata.org/going-off-the-map/#keep-and-discard
