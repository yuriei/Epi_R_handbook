...
<span style="color: black;">**_NOTE:_** The column name "...24" was changed to "x24".</span>
...
```{r, eval=F}
linelist_raw <- openxlsx::readWorkbook("ebola_simulated.xlsx", fillMergedCells = TRUE)
```
...
linelist_raw_names <- import("ebola_simulated.xlsx") %>% names()  # save true column names
...
```{r, eval=F}
# For csv files
linelist_raw_names <- import("ebola_simulated.csv") %>% names() # save true column names
...
In the linelist, there are a few columns we do not need: `row_num`, `merged_header`, and `x24`. Remove them by adding a `select()` command to the cleaning pipe chain:  
...
linelist <- linelist_raw %>%
    ...
    # remove column
    select(-c(row_num, merged_header, x24))
```
...
### Modify class for multiple columns
...
* The `r nrow(linelist %>% filter(hospital %in% c("Hospital A", "Hospital B") | date_onset < as.Date("2013-06-01")))` rows with onset in 2012 and 2013 at hospitals A, B, and Port, *including*:  
