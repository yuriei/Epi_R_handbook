...
In this page we will focus on converting an existing `ggplot()` plot into an interactive plot with **plotly**. You can read more about **shiny** in the [Shiny and dashboards] page. What will be obvious but is worth mentioning nonetheless is that interactive plots are only useable in HTML format Rmarkdown   documents, not PDF or Word documents.

Below is a basic epicurve that has been transformed to be interactive using the integration of **ggplot2** and **plotly** (hover your mouse over the plot, or click items in the legend). 
...
weekly_deaths <- linelist %>%
  dplyr::group_by(epiweek = floor_date(date_onset, "week")) %>%  # create and group data by epiweek column
  dplyr::summarise(                                              # create new summary data frame:
...
You can make almost any `ggplot()` plot interactive, including heat tiles. In the page on [Heat tiles] you can read about how to make the below plot, which displays the proportion of days per week that certain facilities reported data to their province.  
...
agg_weeks <- facility_count_data %>% 
  dplyr::filter(District == "Spring",
         data_date < as.Date("2019-06-01")) %>% 
  dplyr::mutate(week = aweek::date2week(
    data_date,
    start_date = "Monday",
    floor_day = TRUE,
    factor = TRUE)) %>% 
  dplyr::group_by(location_name, week, .drop = F) %>%
  dplyr::summarise(
    n_days          = 7,
    n_reports       = dplyr::n(),
  ...
  dplyr::right_join(tidyr::expand(., week, location_name)) %>% 
  dplyr::mutate(week = aweek::week2date(week))
