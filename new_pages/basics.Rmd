...
This page reviews the essentials of R. This page is not intended to be a comprehensive tutorial, but it provides the basics and can be useful for refreshing your memory. The section on [Learning](#learning) links to more comprehensive tutorials.  
...
**RStudio** - RStudio is a Graphical User Interface (GUI) for easier use of **R**. Read more [in this section](#rstudio).  
...
**Scripts** - A script is the document file that hold your commands. Read more [in this section](#scripts)
...
Writing your own functions is an incredibly useful skill, with many advantages:

* They facilitate modular programming, that is, they allow the separation of code in to independent and manageable pieces. 
* With custom functions, you can give these pieces of code memorable names that are easy to interpret.
* They can also replace repetitive copy and pasting which can be error prone.

Using the previous toy example, to create the oven_bake function, the syntax may look something like this:

```{r basics_functions_image2, echo=F, out.width = "70%", fig.align = "center"}
knitr::include_graphics(here::here("images", "Function_Syntax_Bread_Example.png"))
```

Here is an example of a real function, where we are simulating a staffing calculator for covid-19 contact tracing calls. The inputs, number_of_daily_covid_cases, call_length_per_case, call_length_per_contact and working_hours_per_day are used to calculate the number of staff required (per day) to make the calls. Defaults are set in the round brackets; the length of time to call a contact is 15 mins (0.25 hours), to call a case is 30 mins (0.5 hours) and the staff working day is 8 hours. In the curly brackets we perform the calculation:

```{r message=FALSE, warning=FALSE, out.width = "75%", out.height="75%"}
contact_tracing_staff_calculator <- function(daily_covid_cases, call_length_per_case = 0.5, call_length_per_contact = 0.25, working_hours_per_day = 8){ #the defaults are set in the round brackets
  
  case_hours <- daily_covid_cases * call_length_per_case 
  contact_hours <- daily_covid_cases * call_length_per_contact
  
  staff_required <- (case_hours + contact_hours)/working_hours_per_day
...
As the number of daily covid cases is not set in the function we must specify it when we call the function. To calculate the number of staff required if the number of daily covid cases is 5,500 the function is called as follows:

```{r message=FALSE, warning=FALSE, out.width = "75%", out.height="75%"}
contact_tracing_staff_calculator(daily_covid_cases = 5500)
```

Writing functions is covered further in chapter 44, [Writing functions].
...
In summary: to use the functions available in a R package, 2 steps must be implemented:  
...
It is worth noting that beginner R users can also use R Markdown - do not be intimidated! To learn more, see the handbook page on [R Markdown reports] documents.  
...
A common, efficient, and trouble-free way to manage your working directory and file paths is to combine these 3 elements:  

1) A R Project to store all your files  
2) The **here** package to locate files  
3) The **rio** package to import/export files  
...
<span style="color: red;">**_DANGER:_** Setting a working directory with `setwd()` *can* be "brittle" if the file path is specific to one computer. Instead, use file paths relative to a R Project root directory (with the **here** package). </span>  
