...
1) Create a new 'week' column within `mutate()`, using `date2week()` from the **aweek** package:  
    + use `week_start = ` to set the weekday start of the week (e.g. "Monday", "Sunday") - the default is Monday  
    + use `floor_date = TRUE` so the output is "YYYY-Www" (not the default "YYYY-Www-d" including day of the week)  
    + use `factor = TRUE` so all possible weeks in the range are included as factor levels, so all weeks will appear *when summarized*.  
    + use `numeric = TRUE` if you want only the week number (note this will not distinguish between years)
2) Use `.drop = F` in the subsequent `group_by()` command, preserving all weeks even those with no cases.  
...
1) Create a dataframe, by date x-axis, with a column for each element (bar height, or line height)  
2) 
