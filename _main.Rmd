...
<span style="color: brown;">**This is a free open-access R reference book for applied epidemiologists and public health practitioners.**</span>

**This book strives to:**  

* Serve as a quick reference guide - not as a textbook or comprehensive R training  
...
* Use clear language with step-by-step instructions and helpful code annotation  
...
A common, efficient, and trouble-free way to use R and manage your working directory is to combine these 3 elements: An R project, the **here** package, and the `import()` function to load your data. See the page on [R projects] for more details.  


<!-- ======================================================= -->
### Set by command {-}

Use the command `setwd()` with the desired folder filepath in quotations, for example:  
...
<!-- ======================================================= -->
### Set manually {-}
To set the working directory manually (point-and-click):  

From RStudio click: Session / Set Working Directory / Choose Directory  

(you will have to do this each time you open RStudio)
...
In an R markdown script, the default working directory is the folder the Rmarkdown file (`.Rmd`) is saved within. If using an R project and **here** package, this does not apply.  

If you want to change the working directory of a stand-alone R markdown, using `setwd()` will only apply to that specific code chunk. To make the change for all code chunks in an R markdown, edit the setup chunk to add the `root.dir = ` parameter, such as below:
...
One possible exception to this is if you need to load data from across several separate folder locations. In this case you can still use an R project and relative filepaths for your scripts and outputs, but you may use absolute filepaths when importing these data.
...
When importing or exporting data, you must provide a file path. You can do this one of two ways:  

1) Providing the "full"/"absolute" file path  
2) Provide a "relative" file path - the location *relative to* a project root directory  



### "Absolute" file paths {-}  

This is an example of a full file path, provided to `import()` like this in quotes:  

```{r, eval=F}
import("C:/Users/Pierre/My Documents/epiproject/data/linelists/ebola_linelist.xlsx")
```

Full file paths are helpful if loading data from a distant folder, but **this absolute filepath can break if the script is run on a different computer.**  
...
Below, the file location is given *relative to* an R project root directory (e.g. the R project is in the "epiproject" folder from the above full file path)

```{r, eval=F}
import(here("data", "linelists", "ebola_linelist.xlsx"))
```

The package **here** and its function `here()` locate files on your computer in relation to the root directory of an R project. See the page on [R projects] for more informaton on how they work togther. This is how `here()` works:  

* When the **here** package is first loaded, it places a small file called "here" in the root-level folder of your R project as a "benchmark" or "anchor" for all other files in the project.  
* In your script, if you want to import or reference a file saved in your R project’s folders, you use the function `here()` to tell R where the file is located *in relation to that anchor*. `here()` can be used for both importing and exporting.  

If you are unsure where “here” root is set to, run the function `here()` with the empty brackets:
...
Below is an example of importing the file “linelist.xlsx” which is located in the anchor “here” folder. In this case, all you have to do is provide the name of the file in quotes with the file extension.

```{r, eval=F}
linelist <- import(here("linelist_raw.xlsx"))
```

If the file is within a subfolder - let’s say a "linelist" folder within a “data” folder - write these folder names in quotes, separated by commas, as below:

```{r, eval=F}
linelist <- import(here("data", "linelists", "ebola_linelist.xlsx"))
```

Using the `here()` command produces a character file path, which is then processed by the `import()` function.
...
When you import a dataset, you are generally doing the following:  

1. Creating a new, named object in your R environment of class "data frame"  
2. Defining that new object as the imported dataset  


<!-- ======================================================= -->
### Import from filepath {-}  

As described above, the function `import()` from the package **rio** accepts a file path within quotation marks. A few things to note:  

* Slashes must be forward slashes, as in the code shown. This is *NOT* the default for Windows filepaths.  
* File paths that begin with double slashes (e.g. "//...") will likely **not be recognized by R** and will produce an error. Consider moving these files to a "named" or "lettered" drive that begins with a letter (e.g. "J:" or "C:"). See the section on using Network Drive for more details on this issue.  
* Do not worry about slash direction if using relative file paths with **here** 
...
If you are trying to import a list, and you want it to be imported with the original structure (e.g. a list of lists), use `import_list()` from **rio**:  
...
An R project enables your work to be bundled in a self-contained folder with the working directory being the project's root folder. Within the project, all the relevant data files, figures/outputs, scripts, and history can be stored in sub-folders. This makes it easy to share R projects across computers.  
...
3) The **rio** for importing/exporting  
     ...
     - See the page on [Import and export] for more information  
...
Also consider a version control system. It could be something as simple as having dates on the names of scripts (e.g. "transmission_analysis_2020-10-03.R") and having a header at the top of each script with a description, tags, change log, and author list.  
...
One useful tip is that you can search across an entire project or folder using the "Find in Files" tool (Edit menu). It can search and even replace strings across multiple files.  
