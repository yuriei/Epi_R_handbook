...
We will use the standard, cleaned linelist for all analyses in this section. If you want to download the data to follow step-by-step, see instructions in the [Datasets and versions used] page.
...
We will create an incidence object. The page on [Epidemic curves] for more examples with the **incidence2** package. The incidence object consists of a tibble with dates and their respective case counts. We `rename()` the columns to what is expected by `estimate_R()` in a later step.  
