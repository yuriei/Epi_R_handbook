...
To demonstrate data cleaning, this page begins by importing a raw case linelist dataset, and proceeds step-by-step through the cleaning process. In the R code, this manifests as a "pipe" chain, which references the **magrittr** "pipe" operator ` %>%` which passes a dataset from one operation to the next.  

The essential R functions demonstrated in this page mostly belong to the **tidyverse** family of R packages, and include:  

Function       | Utility                               | Package
---------------|---------------------------------------|------------------------------
` %>% `|"pipe" (pass) data from one function to the next|**magrittr** (also **tidyverse**)
`mutate()`|create, transform, and re-define columns|**dplyr** (also **tidyverse**) 
`select()`|keep, remove, select, or re-name columns| "
...
`across()`|transform multiple columns at one time| " 
**tidyselect** functions|use logic to select columns|**tidyselect** (also **tidyverse**)  
`filter()`|keep certain rows| " 
`distinct()`|de-duplicate rows| " 
`rowwise()`|operations by/within each row| "  
`add_row()`|add rows manually| " 
`recode()`|re-code values in a column| " 
`case_when()`|re-code values in a column using more complex logical criteria| " 
...
`clean_data()`|re-code/clean using a data dictionary|**linelist**
...
This handbook emphasizes use of the above functions from the **tidyverse** R package for data management. However, you may encounter an alternative framework from the **data.table** R package with operators like `:=`. This approach and syntax is briefly explained in the [Data Table] page.  
...
Here we import the raw .xlsx case linelist using the `import()` function from the package **rio**, and save it as the data frame `linelist_raw`. If you want to follow along, <a href='https://github.com/epirhandbook/Epi_R_handbook/raw/master/data/case_linelists/linelist_cleaned.rds' class='download-button'>click to download the "clean" linelist</a> (as .rds file).  
...
You can view the first 50 rows of the the original "raw" dataset below. You can use the **base** R function `head(n)` to view just the first `n` rows in the R console.  
...
<span style="color: black;">**_NOTE:_** To reference a column name that includes spaces, surround the name with back-ticks, for example: linelist$`` `r '\x60infection date\x60'` ``. note that on your keyboard, the back-tick (`) is different from the single quotation mark (').</span>
...
### "tidyselect" helper functions {.unnumbered}  
...
### Transform multiple columns {.unnumbered}
...
Here is an example *outside the context of a data frame*: Let us say you have two vectors, one containing village of detection and another containing village of residence. You can use coalesce to pick the first non-missing value for each index:  
...
The easiest way to solve this is to use the **forcats** function `fct_explicit_na()` takes a factor column and converts `NA` values to the character "(Missing)".  
...
Use the package **linelist** and it's function `clean_data()` to clean a linelist with a *cleaning dictionary*.  
...
2) Store names of any columns that you want to "protect" from the changes.  

```{r}
protected_cols <- c("case_id", "source")
```

3) Run `clean_data()`, specifying the cleaning dictionary. Protected columns must be provided to `clean_data()` as a numeric or logical vector, so you will see use of `names(.)` in the command below (the dot means the data frame). Note this function can take a long time to run.  
...
Scroll to see how values have changed - particularly `gender` (lowercase to uppercase), and all the symptoms columns have been transformed from yes/no to 1/0.  
...
<span style="color: orange;">**_CAUTION:_** `clean_data()` from **linelist** package will implement standard cleaning operations on values in your data unless those columns are protected. Thus you may see changes in values containing dashes "-" or  "." unless protected.</span>
...
You may want to assign `NA` values a label such as "Missing". Because the new column is class Factor (restricted values), you cannot simply mutate it with `replace_na()`. Instead, use `fct_explicit_na()` from **forcats** as explained in the [Factors] page.   
...
In this case, the logical statement is `!is.na(case_id)`, which is asking whether the value in the column `case_id` is **not** missing (`NA`). When placed in front of a function like `is.na()` the operator `!` inverts the criteria. Thus, rows where `case_id` is **not** missing are kept.  
...
More complex logical statements can be constructed using parentheses `( )`, OR `|`, and AND `&` operators. An example is below:  
...
We start with a linelist of `r `nrow(linelist)`. Here is our filter statement:  
