...
pacman::p_load(
  ...
  aweek,      # flexibly converts dates to weeks, and vis-versa
...
`as.Date()` requires that the user *specify the **existing** format of the raw character date*, so it can understand, convert, and store each element (day, month, year, etc.) correctly. [Read more online about `as.Date()`](https://www.stat.berkeley.edu/~s133/dates.html).  
...
If you are unsure or confused about the class of your data (e.g. POSIXct, etc.) it can be easiest to first convert the column to class Character with `as.character()`, and then convert it to class Date.  
