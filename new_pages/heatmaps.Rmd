...
We begin by importing a dataset of daily malaria reports from many facilities. The reports contain a date, province, district, and malaria counts. See the page on [Datasets and versions used] for informaton on how to download these data. Below are the first 30 rows:  
...
Now we group the data into facility-weeks and summarise them to produce statistics per facility-week. See the page on [Descriptive analysis] for tips. The grouping itself doesn't change the dataframe, but it impacts how the subsequent summary statistics are calculated.  
