...
An Automated Programming Interface (API) is commonly used to directly request clean data from a website. You can use R with the packages **httr** and **jsonlite** to pull directly via an API.  
...
See the page on [Iteration and loops] for examples of how to import and combine multiple files, or multiple Excel workbook files. That page also has examples on how to split a data frame into parts and export each one separately, or as named sheets in an Excel workbook.  
...
Assuming  dates are written in the same date general format (e.g. Year Month Day) you can use **lubridate**'s flexible `ymd()` function (or `dmy()` or `mdy()`) to convert them to dates. For these functions, the dashes, spaces, or slashes do not matter, only of order of the numbers. Read more in [Working with dates].  
...
```{r}
here("data", "linelists", latest_file) 
```
...
```{r, eval=F}
# import
import(here("data", "linelists", latest_file)) # import 
```
...
Below, we provide the folder of interest to **fs**'s `dir_info()`. In this case, the folder of interest is the sub-folder "linelists" within the folder "data" within the R project. The result is a data frame with one line per file and columns for `modification_time`, `path`, etc. You can see a visual example of this in the page on [Directory interactions].    
