...
* **R markdown combined with pre-processing**: This is when you pre-process objects that feed into your R markdown. For instance, you can create a script which loads the packages, loads the data, cleans the data, and even created the graphs of interest. Then the markdown itself will only be used for the final step to produce the output with all the pre-processed objects. This is much easier to de-bug if something goes wrong.
...
The workflow also concerns the overall folder structure, such as having an 'output' folder for created documents and an 'input' folder for cleaned data. We do not go into further detail of further structure here, but the 'organising routine reports' discussed this further. 
...
Call the Rmd file using the ‘render’ function of the rmarkdown package in a separate script, making sure to refer to the correct file in the correct directory. In its simplest form, this will save to the same folder as your Rmd file, and with the same file name aside from the file extension. For instance “create_output.Rmd” when knitted will create “create_output.docx” if you are knitting to a word document. 
...
When you have the Rmd file open, press the 'knit' button at the top of the file. This is RStudio’s button for the `render` function of **rmarkdown**. This is only compatible with a self-contained R markdown, where all the needed components are within the file.   
