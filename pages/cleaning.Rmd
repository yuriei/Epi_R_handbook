...
Also note that in the raw data, the two final columns names were one merged cell with the name "header_merged". The `import()` function used the name for the first of the two columns, and assigned the second column the name "...23" (referring to the 23rd column).  
...
Note that the column name "...23" was changed to "x23".  
...
You can also rename by column position, instead of column name, as below:  
...
If you are importing an Excel sheet and there is a missing column name, it will replace it with a value like "...1" or "...2".


#### Merged Excel column names  

Merged cells in an Excel file can cause problems, especially in the column names. Do your best to ensure the data you recieve is "tidy" without merged cells.  
...
!!! Warning: If column names are merged, you will end up with duplicate column names, which you will need to fix manually - R does not work well with duplicate column names. You can re-name them by referencing their position (e.g. column 5), as explained in the section above on manual column name cleaning.  


#### Skip import of column names  

Sometimes, you may want to avoid importing a row of data. Using `import()` from the **rio** package, you can do this with the argument `skip = `. Provide the number of rows you want to skip.
...
In other cases you may want to avoid importing the *second* row of data, for example if it is a data dictionary row.  This can be problematic because it can result in all columns being imported as class "character". To address this, you may need to import the data twice.  

1) Import the data in order to store the correct column names  
2) Import the data again, skipping the first *two* rows  
3) Combine the correct names onto the dataframe

Save just the column names  

```{r, eval=F}
linelist_raw_names <- rio::import(here::here("data", "ebola_simulated.xlsx")) %>% 
  names()
```

How you re-attach the names depends on which kind of file (.csv, .tsv, .xlsx, etc.). If using **rio**'s `import()` function, understand which function **rio** is using to import your data, and use the appropriate argument to skip lines and/or add column names. See the handbook page on importing data (LINK) for details on **rio**.  
...
linelist_raw <- import("ebola_simulated.xlsx", skip = 2, col_names = linelist_raw_names)
```
...
linelist_raw <- import("ebola_simulated.csv", skip = 2, col.names = linelist_raw_names)
```
...
You may want to combine two header rows into one:  
...
<!-- ======================================================= -->
## Changing multiple variables at one time
